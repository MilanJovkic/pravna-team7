...
class VerdictAkomaExporter:
	...
	def _build_meta(
		...
		SubElement(
			expression,
			"FRBRthis",
			value=f"/akn/{self.country_code}/judgment/{case_id}/sr@{metadata.date or self.current_date}/!main",
		)
		SubElement(
			expression,
			"FRBRuri",
			value=f"/akn/{self.country_code}/judgment/{case_id}/sr@{metadata.date or self.current_date}",
		)
		...
		SubElement(
			manifestation,
			"FRBRthis",
			value=f"/akn/{self.country_code}/judgment/{case_id}/sr@{metadata.date or self.current_date}/!main.xml",
		)
		SubElement(
			manifestation,
			"FRBRuri",
			value=f"/akn/{self.country_code}/judgment/{case_id}/sr@{metadata.date or self.current_date}/!main.akn",
		)
		...
		for idx, law in enumerate(applied_laws or [], 1):
			...
			SubElement(references, "TLCLaw", eId=law_id, href=self._law_href(law), showAs=law)
	...
	def _write_pretty_xml(
		...
		"""Writes XML to file with formatting and optional DB insert."""
		...
		if self.enable_db_insert:
			try:
				...
				print(f"  [!] Nije uspeo upis u bazu za {safe_case_id}: {str(exc)[:50]}")
	...
	def _insert_case_to_db(self, case_number: str, factual_state: Dict[str, list], outcome: str):
		...
		try:
			...
			raise Exception(f"Database insert failed: {exc}")